package com.dtwave.dipper.asset.mllib.isotonicregression
...
object Isotonic_Regression {
  ...
  def main(args: Array[String]) {
    // 构建Spark对象
    val conf = new SparkConf().setAppName("Isotonic_Regression").setMaster("local")
    val sc = new SparkContext(conf)
    Logger.getRootLogger.setLevel(Level.WARN)

    // 读取样本数据
    val data = sc.textFile("/Users/hulb/data/mllib/data/sample_isotonic_regression_data.txt")
    ...
    // 保存模型
    val ModelPath = "/Users/hulb/data/mllib/model/Isotonic_Regression_Model"
