...
object ExecBucket {
  ...
  /**
    ...
    * @param partitionName
    */
  def bucket(spark: SparkSession, tableName: String, columnName: String, min: Double, max: Double): util.HashMap[String, String] = {
    ...
    for (row <- mapResult) {
      ...
      println(bucketIndexToString + " : " + count.get)
      disMap.put(bucketIndexToString, count.get)
